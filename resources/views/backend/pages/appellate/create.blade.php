@section('page-title')
    @if(isset($case)) Update Appellate Case @else Create Appellate Case @endif
@endsection
        ...
        <div class="p-5">
            <form id="submit-form" method="POST">
                ...
                <div>
                    ...
                    <input type="text" id="case_no" name="case_no" class="input w-full border mt-2" placeholder="e.g. CP 1234/2026" value="{{ $case->case_no ?? '' }}" required>
                </div>
                ...
                <a href="{{ route('dashboard.supreme-court.appellate.index') }}" class="button bg-gray-200 text-gray-700 mt-5 ml-2 inline-block">Cancel</a>
            </form>
        </div>
        <!-- END: Form -->
    </div>
    ...
    <script>
        $(document).ready(function(){
            $('#submit-form').on('submit', function (e) {
                ...
                @if(isset($case))
                    var actionurl = "{{ route('dashboard.supreme-court.appellate.update', $case->id) }}";
                @else
                    var actionurl = "{{ route('dashboard.supreme-court.appellate.store') }}";
                @endif
                    ...
                    success: function(response) {
                        if(response.status === 'success'){
                            Swal.fire(
                                ...
                                window.location.href = "{{ route('dashboard.supreme-court.appellate.index') }}";
                            });
                        } else {
                            Swal.fire(
                                'Error!',
